...
x_CL_min = x(posicion_CL_min);
z_CL_min = z(posicion_CL_min);

[CL_maximo,posicion_CL_max] = max(CL_req); % para encontrar la posición de CL máximo 
x_CL_max = x(posicion_CL_max);
z_CL_max = z(posicion_CL_max);

% Evolución de n

[n_minimo,posicion_n_min] = min(n_req); % para encontrar la psosición del n mínimo
x_n_min = x(posicion_n_min);
z_n_min = z(posicion_n_min);

[n_maximo,posicion_n_max] = max(n_req); % para encontrar la posición de n máximo 
x_n_max = x(posicion_n_max);
z_n_max = z(posicion_n_max);

% Empuje requerido

[T_maximo, posicion_T_max] = max(T_req); % para encontrar la posición del empuje máximo 

x_T_max = x(posicion_T_max);
z_T_max = z(posicion_T_max);
phi_T_max = phi_deg(posicion_T_max);

%posiciones = find(T_req==15800) % al no tener el valor justo de 15800 N,
%no se puede encontrar con esta funcion.

% buscando en el vector T_req, T=15800 se encuentra entre las posiciones 37
% y 38 por primera vez
x_valor_T_max_1_1 = x(37);
x_valor_T_max_1_2=x(38);

z_valor_T_max_1_1 = z(37);
z_valor_T_max_1_2=z(38);

phi_valor_T_max_1_1 = phi_deg(37);
phi_valor_T_max_1_2 = phi_deg(38);


% buscando en el vector T_req, T=15800 se encuentra entre las posiciones
% 445 y 446 por segunda vez 
 
x_valor_T_max_2_1 = x(445);
x_valor_T_max_2_2=x(446);

z_valor_T_max_2_1 = z(445);
z_valor_T_max_2_2=z(446);

phi_valor_T_max_2_1 = phi_deg(445);
phi_valor_T_max_2_2 = phi_deg(446);