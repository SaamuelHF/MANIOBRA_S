...
%  de phi=pi a phi=2pi, cubriendo correctamente el ascenso.

CD_req = CD0 + k * CL_req.^2;
D_req  = q * S * CD_req;
T_req  = D_req + W * curv.* sin(phi);
...
%% Código para reponder a las preguntas planteadas 

% Evolución temporal o angular de C_L

[CL_minimo,posicion_CL_min] = min(CL_req); % para encontrar la psosición del CL mínimo
x_CL_min = x_tray(posicion_CL_min);
z_CL_min = z_tray(posicion_CL_min);

[CL_maximo,posicion_CL_max] = max(CL_req); % para encontrar la posición de CL máximo 
x_CL_max = x_tray(posicion_CL_max);
z_CL_max = z_tray(posicion_CL_max);


% Evolución de n

[n_minimo,posicion_n_min] = min(n_req); % para encontrar la psosición del n mínimo
x_n_min = x_tray(posicion_n_min)
z_n_min = z_tray(posicion_n_min)

[n_maximo,posicion_n_max] = max(n_req); % para encontrar la posición de n máximo 
x_n_max = x_tray(posicion_n_max)
z_n_max = z_tray(posicion_n_max)

% posicion_valor_n_min = find(n_req == n_min); % no se puede encontrar
% porque no esta justo el valor de -3.9, lo he buscado en el vector y el
% valor se encuentra entre las posiciones 765 y 766. 
     
x_valor_n_min_1 = x_tray(765)
z_valor_n_min_1 = z_tray(765)

x_valor_n_min_2 = x_tray(766)
z_valor_n_min_2 = z_tray(766)


% Empuje requerido

[T_maximo_AB, posicion_T_max_AB] = max(T_req(1:500)) % para encontrar la posición del empuje máximo del tramo AB
[T_maximo_BC, posicion_T_max_BC] = max(T_req(501:1000))% para encontrar la posición del empuje máximo del tramo BC

x_T_max_AB = x_tray(posicion_T_max_AB)
z_T_max_AB = z_tray(posicion_T_max_AB)
phi_T_max_AB = phi_deg(posicion_T_max_AB)

x_T_max_BC = x_tray(posicion_T_max_BC+500)
z_T_max_BC = z_tray(posicion_T_max_BC+500)
phi_T_max_BC = phi_deg(posicion_T_max_BC+500)
